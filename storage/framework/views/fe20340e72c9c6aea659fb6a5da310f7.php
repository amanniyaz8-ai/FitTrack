<?php $__env->startSection('title', 'Мониторинг'); ?>
...
<div class="flex items-center justify-between mb-4">
    <h1 class="text-lg md:text-xl font-bold" style="color: #0f2035;"><?php echo e(__('app.dashboard_title')); ?></h1>
    <span class="text-xs text-gray-400 hidden sm:flex items-center gap-1">
        <i class="fas fa-sync-alt"></i>
        <?php echo e(app()->getLocale() === 'kk' ? 'Жаңартылды' : 'Обновлено'); ?>: <?php echo e(now()->format('H:i:s')); ?> · <span id="countdown">60</span> <?php echo e(app()->getLocale() === 'kk' ? 'сек' : 'сек'); ?>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
    <div class="bg-white rounded-xl shadow p-3 md:p-5 border-l-4" style="border-left-color: #f97316;">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-500 text-xs md:text-sm"><?php echo e(__('app.client_base')); ?></p>
                <p class="text-2xl md:text-3xl font-bold" style="color: #0f2035;"><?php echo e($totalClients); ?></p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow p-3 md:p-5 border-l-4 border-blue-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-500 text-xs md:text-sm"><?php echo e(__('app.active_packages')); ?></p>
                <p class="text-2xl md:text-3xl font-bold" style="color: #0f2035;"><?php echo e($activePackages); ?></p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow p-3 md:p-5 border-l-4 border-green-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-500 text-xs md:text-sm"><?php echo e(__('app.sessions_today')); ?></p>
                <p class="text-2xl md:text-3xl font-bold" style="color: #0f2035;"><?php echo e($todaySessions); ?></p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow p-3 md:p-5 border-l-4 border-yellow-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-500 text-xs md:text-sm"><?php echo e(app()->getLocale() === 'kk' ? 'Аяқталуда' : 'Заканчиваются'); ?></p>
                <p class="text-2xl md:text-3xl font-bold" style="color: #0f2035;"><?php echo e($expiringPackages->count()); ?></p>
            </div>
...
<?php if($expiringPackages->count() > 0): ?>
<div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
    <h3 class="font-semibold text-yellow-800 mb-2">
        <i class="fas fa-exclamation-triangle mr-2"></i><?php echo e(__('app.expiring_packages')); ?>

    </h3>
    <div class="space-y-2">
        <?php $__currentLoopData = $expiringPackages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pkg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="flex items-center justify-between bg-white rounded-lg px-4 py-2.5 shadow-sm">
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                <a href="<?php echo e(route('clients.show', $pkg->client)); ?>" class="font-medium hover:text-orange-500 transition" style="color: #0f2035;">
                    <?php echo e($pkg->client->full_name); ?>

                </a>
                <span class="text-gray-500 text-sm"><?php echo e(__('app.sessions_left')); ?>:
                    <strong class="<?php echo e($pkg->scheduled_count === 1 ? 'text-red-600' : 'text-orange-500'); ?>">
                        <?php echo e($pkg->scheduled_count); ?>

                    </strong>
                </span>
                <span class="text-gray-400 text-xs">
                    <i class="fas fa-calendar-alt mr-1"></i><?php echo e(__('app.payment_date')); ?>:
                    <span class="text-gray-600 font-medium"><?php echo e($pkg->payment_date->format('d.m.Y')); ?> г.</span>
                </span>
            </div>
            <a href="<?php echo e(route('packages.create', $pkg->client)); ?>" class="text-sm text-white px-3 py-1 rounded-lg transition ml-3 flex-shrink-0" style="background-color: #f97316;" onmouseover="this.style.backgroundColor='#ea580c'" onmouseout="this.style.backgroundColor='#f97316'">
                + Пакет
            </a>
        </div>
...
<div class="bg-white rounded-xl shadow overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-3" style="background-color: #0f2035;">
        <h2 class="text-white font-semibold text-lg">
            <i class="fas fa-calendar-alt mr-2" style="color: #fb923c;"></i><?php echo e(__('app.upcoming_sessions')); ?>
    ...
    <?php if($upcomingSessions->isEmpty()): ?>
        <div class="text-center py-12 text-gray-400">
            ...
            <p><?php echo e(app()->getLocale() === 'kk' ? 'Бүгін және ертең жаттығу жоқ' : 'Нет запланированных занятий на сегодня и завтра'); ?></p>
        </div>
    <?php else: ?>
    <?php
        $todaySessions2 = $upcomingSessions->filter(fn($s) => $s->scheduled_date->isToday());
        $tomorrowSessions = $upcomingSessions->filter(fn($s) => !$s->scheduled_date->isToday());
    ?>

    
    <?php if($todaySessions2->isNotEmpty()): ?>
    <div class="px-4 py-2 bg-orange-50 border-b border-orange-100 flex items-center justify-between">
        <span class="text-xs font-semibold text-orange-500 uppercase tracking-wide"><?php echo e(__('app.today')); ?> — <?php echo e(now()->format('d.m.Y')); ?></span>
        <div class="flex items-center gap-2">
            <button id="btn-edit-mode" onclick="enterEditMode()"
                ...
                <i class="fas fa-edit text-xs"></i> <?php echo e(__('app.edit')); ?>
            ...
            <div id="edit-mode-actions" class="hidden flex items-center gap-1">
                <button onclick="openDashAddModal('today')"
                    ...
                    <i class="fas fa-plus text-xs"></i> <?php echo e(__('app.add')); ?>
                ...
                <button onclick="exitEditMode()"
                    ...
                    <i class="fas fa-check text-xs"></i> <?php echo e(__('app.done')); ?>
    ...
    <div class="divide-y divide-gray-50">
        <?php $__currentLoopData = $todaySessions2; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $isToday = true; $dayLabel = __('app.today_label'); $dateLabel = $session->scheduled_date->format('d.m.Y'); ?>
        <div id="session-row-<?php echo e($session->id); ?>"
             class="dash-session-row today-row px-4 py-3 flex items-center gap-3 transition <?php echo e($session->status === 'completed' ? 'bg-green-50' : 'hover:bg-gray-50'); ?>">
            <input type="checkbox" class="dash-session-cb hidden w-4 h-4 rounded cursor-pointer shrink-0"
                   value="<?php echo e($session->id); ?>" style="accent-color:#f97316;" onchange="dashUpdateTodayBar()">

                
                <?php if($session->status === 'completed'): ?>
                    
                    <form method="POST" action="<?php echo e(route('sessions.update', $session)); ?>" class="shrink-0">
                ...
                <button type="button"
                    onclick="openTimePicker(this, '<?php echo e(route('sessions.update', $session)); ?>', '<?php echo e($session->status); ?>', '<?php echo e($session->scheduled_time ? \Carbon\Carbon::parse($session->scheduled_time)->format('H:i') : ''); ?>')"
                    ...
                    <p class="text-sm font-bold tabular-nums group-hover:opacity-70 transition"
                       style="letter-spacing:0.02em; <?php echo e($session->scheduled_time ? 'color:#f97316;' : 'color:#94a3b8;'); ?>">
                        <?php echo e($session->scheduled_time ? \Carbon\Carbon::parse($session->scheduled_time)->format('H:i') : '—:—'); ?>

                    </p>
                    <p class="text-xs <?php echo e($isToday ? 'font-semibold' : 'text-gray-400'); ?>"
                       style="<?php echo e($isToday ? 'color:#f97316;' : ''); ?>">
                        <?php echo e($dayLabel); ?>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5">
                        ...
                        <a href="<?php echo e(route('clients.show', $session->client)); ?>"
                           class="flex-1 min-w-0 font-medium hover:text-orange-500 transition truncate <?php echo e($session->status === 'completed' ? 'line-through text-gray-400' : ''); ?>"
                           style="<?php echo e($session->status !== 'completed' ? 'color:#0f2035;' : ''); ?>">
                            <?php echo e($session->client->full_name); ?>
                        ...
                        <?php
                            ...
                            $pricePerSession = ($pkg && $pkg->total_sessions > 0)
                                ...
                                : 0;
                        ?>
                        <?php if($pricePerSession > 0): ?>
                        <span class="text-xs font-semibold shrink-0 hidden sm:inline <?php echo e($session->status === 'completed' ? 'text-green-600' : 'text-gray-400'); ?>">
                            <?php echo e(number_format($pricePerSession, 0, '.', ' ')); ?> ₸
                        </span>
                        <?php endif; ?>
                        
                        <div class="flex items-center gap-1 shrink-0">
                            <?php if($session->status === 'scheduled'): ?>
                                <form method="POST" action="<?php echo e(route('sessions.update', $session)); ?>">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                    <input type="hidden" name="status" value="missed">
                                    <button type="submit" title="<?php echo e(__('app.missed_short')); ?>"
                                        ...
                                        <span class="hidden md:inline"><?php echo e(__('app.missed_short')); ?></span>
                                    </button>
                                </form>
                                <button type="button" title="<?php echo e(__('app.cancelled_short')); ?>"
                                    onclick="openDashReschedule(<?php echo e($session->id); ?>, '<?php echo e($session->scheduled_date->format('d.m.Y')); ?>')"
                                    ...
                                    <span class="hidden md:inline"><?php echo e(__('app.cancelled_short')); ?></span>
                                </button>
                            <?php elseif($session->status === 'completed'): ?>
                                <span class="text-xs px-2 py-1 rounded-full bg-green-100 text-green-700 font-medium whitespace-nowrap">
                                    <i class="fas fa-check mr-1"></i><span class="hidden sm:inline"><?php echo e(__('app.came')); ?></span>
                                </span>
                            ...
                            <?php endif; ?>
                            <a href="<?php echo e(route('packages.sessions', $session->package)); ?>"
                    ...
                    <p class="text-xs text-gray-400 mt-0.5"><?php echo e($dateLabel); ?></p>
                </div>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    ...
    <?php if($tomorrowSessions->isNotEmpty()): ?>
    <div class="px-4 py-2 bg-blue-50 border-t border-b border-blue-100 flex items-center justify-between">
        <span class="text-xs font-semibold text-blue-400 uppercase tracking-wide"><?php echo e(__('app.tomorrow')); ?> — <?php echo e(now()->addDay()->format('d.m.Y')); ?></span>
        <div class="flex items-center gap-2">
    ...
    <div class="divide-y divide-gray-50">
        <?php $__currentLoopData = $tomorrowSessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $isToday = false; $dayLabel = __('app.tomorrow_label'); $dateLabel = $session->scheduled_date->format('d.m.Y'); ?>
        <div id="session-row-<?php echo e($session->id); ?>"
             class="dash-session-row tomorrow-row px-4 py-3 flex items-center gap-3 transition <?php echo e($session->status === 'completed' ? 'bg-green-50' : 'hover:bg-gray-50'); ?>">
            <input type="checkbox" class="dash-session-cb hidden w-4 h-4 rounded cursor-pointer shrink-0"
                   value="<?php echo e($session->id); ?>" style="accent-color:#3b82f6;" onchange="dashUpdateTomorrowBar()">
            ...
            <button type="button"
                ...
                <p class="text-xs text-gray-400"><?php echo e($dayLabel); ?></p>
            </button>
...
<div class="mt-4 bg-white rounded-xl shadow overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100" style="background-color: #0f2035;">
        <h2 class="text-white font-semibold text-lg">
            <i class="fas fa-wallet mr-2" style="color: #fb923c;"></i><?php echo e(__('app.day_summary')); ?> — <?php echo e(now()->format('d.m.Y')); ?>
    ...
    <div class="px-4 md:px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="grid grid-cols-3 gap-3 md:flex md:gap-6">
            <div>
                <p class="text-gray-400 text-xs md:text-sm"><?php echo e(__('app.sessions_conducted')); ?></p>
                <p class="text-2xl md:text-3xl font-bold mt-1 whitespace-nowrap" style="color:#0f2035;"><?php echo e($todayCompletedCount); ?></p>
            </div>
            <div class="hidden md:block w-px bg-gray-200"></div>
            <div>
                <p class="text-gray-400 text-xs md:text-sm"><?php echo e(__('app.earned_today')); ?></p>
                <p class="text-2xl md:text-3xl font-bold mt-1 text-green-600 whitespace-nowrap">
                    <?php echo e(number_format($todayEarnings, 0, '.', ' ')); ?> ₸
                </p>
            </div>
            <?php if($todaySessions > $todayCompletedCount): ?>
            <div class="hidden md:block w-px bg-gray-200"></div>
            <div>
                <p class="text-gray-400 text-xs md:text-sm"><?php echo e(__('app.pending_earnings')); ?></p>
                <?php
                ...
                <p class="text-2xl md:text-3xl font-bold mt-1 text-blue-500 whitespace-nowrap">
                    <?php echo e(number_format($pendingEarnings, 0, '.', ' ')); ?> ₸
                </p>
            </div>
            <?php endif; ?>
        </div>

        <?php if($todaySessions > 0): ?>
        <div class="text-right sm:text-right">
            <p class="text-xs text-gray-400 mb-1"><?php echo e(__('app.day_progress')); ?></p>
            <div class="w-full sm:w-48 bg-gray-200 rounded-full h-2.5">
                <?php $dayProgress = $todaySessions > 0 ? round(($todayCompletedCount / $todaySessions) * 100) : 0; ?>
                <div class="h-2.5 rounded-full transition-all" style="width:<?php echo e($dayProgress); ?>%; background:linear-gradient(to right,#fb923c,#16a34a);"></div>
            </div>
            <p class="text-xs text-gray-400 mt-1">
                <?php echo e($todayCompletedCount); ?>

                <?php echo e(str_replace(':total', $todaySessions, __('app.of_trainings'))); ?>
...
<form id="dash-delete-form" method="POST" action="<?php echo e(route('sessions.bulkDelete')); ?>" class="hidden">
...
<div id="dash-add-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center" style="background:rgba(0,0,0,0.5);">
    <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
        <h3 class="text-lg font-bold mb-4" style="color:#0f2035;">
            <i class="fas fa-plus-circle text-orange-400 mr-2"></i><?php echo e(__('app.add_session')); ?>
        ...
        <div class="mb-4 px-4 py-2.5 rounded-xl flex items-center gap-2" id="dash-date-badge" style="background:#fff7ed;">
            ...
            <span class="text-sm text-gray-400" id="dash-date-value"><?php echo e(now()->format('d.m.Y')); ?></span>
        </div>

        
        <form id="dash-add-form" method="POST" action="">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="scheduled_date" id="dash-add-date" value="<?php echo e(now()->format('Y-m-d')); ?>">
            <div class="mb-4">
                ...
                <select id="dash-add-client" name="_client_pkg"
                    ...
                    <?php $__currentLoopData = $addClients; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $pkg = $c->packages->first() ?>
                        <?php if($pkg): ?>
                            <option value="<?php echo e($pkg->id); ?>"><?php echo e($c->full_name); ?></option>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('app.time_optional')); ?></label>
                <div class="flex items-center gap-2">
                    <select id="dash-add-hour"
                        ...
                        <?php for($h = 6; $h <= 22; $h++): ?>
                            <option value="<?php echo e(str_pad($h, 2, '0', STR_PAD_LEFT)); ?>"><?php echo e(str_pad($h, 2, '0', STR_PAD_LEFT)); ?>:00</option>
                        <?php endfor; ?>
                    </select>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/amanniyaz8gmail.com/Desktop/Код/fittrack/resources/views/dashboard.blade.php ENDPATH**/ ?>