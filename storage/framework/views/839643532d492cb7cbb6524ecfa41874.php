<?php $__env->startSection('title', 'Занятия пакета'); ?>
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm">
                <a href="<?php echo e(route('clients.show', $package->client)); ?>" class="hover:underline"><?php echo e($package->client->full_name); ?></a>
                — <?php echo e($package->total_sessions); ?> тренировок
                <?php if($package->is_paid): ?>
...
<div class="grid grid-cols-3 gap-3 mb-6">
    <div class="bg-white rounded-xl shadow p-3 md:p-4 text-center">
        <p class="text-xl md:text-2xl font-bold text-green-600"><?php echo e($completedCount); ?></p>
        <p class="text-gray-400 text-xs md:text-sm">Выполнено</p>
    </div>
    <div class="bg-white rounded-xl shadow p-3 md:p-4 text-center">
        <p class="text-xl md:text-2xl font-bold text-red-500"><?php echo e($missedCount); ?></p>
        <p class="text-gray-400 text-xs md:text-sm">Пропущено</p>
    </div>
    <div class="bg-white rounded-xl shadow p-3 md:p-4 text-center">
        <p class="text-xl md:text-2xl font-bold text-blue-600"><?php echo e($remaining); ?></p>
...
<div class="bg-white rounded-xl shadow p-4 mb-6">
    ...
    <div class="w-full bg-gray-200 rounded-full h-3">
        <div class="h-3 rounded-full transition-all" style="width: <?php echo e($progressPercent); ?>%; background: linear-gradient(to right, #fb923c, #ea580c);"></div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow p-4 mb-6">
    ...
    <form method="POST" action="<?php echo e(route('packages.addSession', $package)); ?>" class="flex flex-col sm:flex-row gap-2 sm:gap-3 sm:items-center">
        <?php echo csrf_field(); ?>
        <input type="date" name="scheduled_date" value="<?php echo e(date('Y-m-d')); ?>"
...
<div class="bg-white rounded-xl shadow overflow-hidden">
    <div class="px-6 py-3 border-b border-gray-100 flex items-center justify-between" style="background-color: #0f2035;">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-white font-medium">Список занятий (<?php echo e($sessions->count()); ?>)</h2>
        </div>
    ...
    <div class="divide-y divide-gray-50" id="sessions-list">
        <?php $__currentLoopData = $sessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
            $ruDays = ['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
            $dayName = $ruDays[$session->scheduled_date->dayOfWeek] ?? '';
        ?>
        <div class="px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3 hover:bg-gray-50 session-row transition" id="session-<?php echo e($session->id); ?>"
             data-id="<?php echo e($session->id); ?>">
            <div class="flex items-center gap-3 flex-1">
                <input type="checkbox" class="session-checkbox w-4 h-4 rounded cursor-pointer shrink-0"
                       value="<?php echo e($session->id); ?>" style="accent-color:#f97316;">
                <span class="text-gray-400 text-sm w-5 text-right shrink-0"><?php echo e($i + 1); ?></span>
                ...
                <button type="button"
                    ...
                    <p class="text-sm font-bold tabular-nums group-hover:opacity-70 transition"
                       style="<?php echo e($session->scheduled_time ? 'color:#f97316;' : 'color:#94a3b8;'); ?>">
                ...
                <div>
                    <p class="font-medium" style="color: #0f2035;">
                        <?php echo e($session->scheduled_date->format('d.m.Y')); ?>

                        <span class="text-gray-400 text-xs font-normal ml-1"><?php echo e($dayName); ?></span>
                    </p>
                    <?php if($session->notes): ?>
                        <p class="text-gray-400 text-xs mt-0.5"><?php echo e($session->notes); ?></p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <form method="POST" action="<?php echo e(route('sessions.update', $session)); ?>">
                    ...
                    <select name="status"
                        onchange="handleStatusChange(this, <?php echo e($session->id); ?>, '<?php echo e($session->scheduled_date->format('d.m.Y')); ?>')"
                        ...
                        <?php echo e($session->status === 'completed' ? 'bg-green-50 border-green-300 text-green-700' :
                        ...
                        <option value="scheduled"  <?php echo e($session->status === 'scheduled'  ? 'selected' : ''); ?>>Запланировано</option>
                        <option value="completed"  <?php echo e($session->status === 'completed'  ? 'selected' : ''); ?>>Отходил</option>
                        <option value="missed"     <?php echo e($session->status === 'missed'     ? 'selected' : ''); ?>>Пропустил</option>
                        <option value="cancelled"  <?php echo e($session->status === 'cancelled'  ? 'selected' : ''); ?>>Отменил</option>
                    </select>
                </form>

                <!-- Delete button -->
                <form method="POST" action="<?php echo e(route('sessions.destroy', $session)); ?>"
                      onsubmit="return confirm('Удалить занятие <?php echo e($session->scheduled_date->format('d.m.Y')); ?>?')">
                    <?php echo csrf_field(); ?>
...
<form id="bulk-form" method="POST" action="<?php echo e(route('sessions.bulkUpdate')); ?>" class="hidden">
    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/amanniyaz8gmail.com/Desktop/Код/fittrack/resources/views/packages/sessions.blade.php ENDPATH**/ ?>