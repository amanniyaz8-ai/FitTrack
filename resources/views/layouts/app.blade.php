    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="flex items-center gap-1 sm:gap-3">
                ...
                @else
                <a href="{{ route('support.cabinet') }}" class="text-gray-300 text-sm hidden md:flex items-center gap-1.5 hover:text-orange-400 transition {{ request()->routeIs('support.cabinet') ? 'text-orange-400' : '' }}">
                    <i class="fas fa-headset"></i>
                    <span>Поддержка</span>
                </a>
                @endif
...
<div id="payment-reminder-banner" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4" style="display:none;">
    <div class="flex items-start gap-3 bg-orange-50 border border-orange-300 rounded-xl px-4 py-3">
        <div class="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5">
            <i class="fas fa-bell text-white text-sm"></i>
        </div>
        <div class="flex-1 min-w-0">
            <p class="font-semibold text-orange-800 text-sm">Напоминание об оплате</p>
        ...
        <button onclick="dismissPaymentReminder()" class="text-orange-400 hover:text-orange-600 flex-shrink-0 mt-0.5">
...
(function() {
    ...
    var key = 'payment_reminder_dismissed_{{ auth()->id() }}';
    var dismissed = localStorage.getItem(key);
    if (dismissed !== today) {
        document.getElementById('payment-reminder-banner').style.display = 'block';
    }
})();
function dismissPaymentReminder() {
    ...
    document.getElementById('payment-reminder-banner').style.display = 'none';
}
</script>
@endif
@endauth
